...
class DynamicArray
{
    ...
    DynamicArray(int size, T value) : Capacity(size), Size(size)
    {
        Arr = new T[Capacity];
    ...
    DynamicArray(int size, std::initializer_list<T> values) : Capacity(size), Size(values.size())
    {
        Arr = new T[Capacity];
        int i = 0;
        for (T value : values) 
...
int main()
{
    ...
    DynamicArray<int>arr3(5, {1, 2, 3, 4, 5});
    arr3.print(); 
